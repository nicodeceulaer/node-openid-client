...
<body onload="javascript:document.forms[0].submit()">
  ...
  decryptIdToken(token, use) {
    console.log(`[decryptIdToken] token:${token}`)
    if (
    ...
    console.log(`[decryptIdToken] header=${header}`)
    ...
    return keystoreOrSecret.then(keyOrStore => jose.JWE.createDecrypt(keyOrStore).decrypt(idToken)
      .then((result) => {
        console.log(`[decryptIdToken] keystoreOrSecret handler result=${result}`)

        if (token instanceof TokenSet) {
          token.id_token = result.payload.toString('utf8');
          console.log(`[decryptIdToken] keystoreOrSecret handler token is TokenSet token:${token}`)
          return token;
        }
        console.log(`[decryptIdToken] keystoreOrSecret handler token is not TokenSet`)
  ...
  validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
    ...
    console.log(`[validatetIdToken]  idToken:${idToken}`)
    ...
    console.log(`[validatetIdToken] JSON parsing idToken::${idToken}`)

    const timestamp = now();
    const parts = idToken.split('.');
    const header = JSON.parse(base64url.decode(parts[0]));
    const payload = JSON.parse(base64url.decode(parts[1]));
    console.log(`[validatetIdToken] JSON parsing done. header:${header}`)
    console.log(`[validatetIdToken] JSON parsing done. payload:${payload}`)
    ...
    console.log(`[validatetIdToken] verification step`)
    ...
    console.log(`[validatetIdToken] verification last step`)
  ...
  refresh(refreshToken) {
    ...
    return this.grant({
      ...
      .then((tokenset) => {
        console.log(`[refresh] grant handler`)
        if (!tokenset.id_token) {
          console.log(`[refresh] grant handler has no id_token - bailing out`)
          return tokenset;
        }
  ...
  authenticatedPost(endpoint, httpOptions) {
    console.log(`[authenticatedPost] endpoint:${endpoint}`)
    return Promise.resolve(this.authFor(endpoint))
      .then((auth) => {
       console.log(`[authenticatedPost] handler auth:${auth}`)
